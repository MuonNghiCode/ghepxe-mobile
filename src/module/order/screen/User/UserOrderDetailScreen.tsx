  ...
  const renderItemsSection = () => (
    <View style={tw`bg-white px-4 py-4 mb-4`}>
      ...
      <View style={tw`flex-row items-center justify-between mb-3`}>
        ...
        <Text style={tw`text-xs text-gray-500`}>Số lượng: {items.length}</Text>
      </View>
      {/* Danh sách kiện hàng */}
      {items.map((item, idx) => (
        <View
          ...
          <View style={tw`flex-1`}>
            ...
            <View style={tw`flex-row items-center mt-1`}>
              ...
              <View style={tw`bg-[#00A982] rounded px-2 py-0.5`}>
                <Text style={tw`text-xs text-white font-semibold`}>10kg</Text>
              </View>
            </View>
            <TouchableOpacity
      ...
      <View style={tw`flex-row items-center py-3 border-t border-gray-100`}>
        ...
        <Text style={tw`ml-2 text-sm text-black`}>Phương thức thanh toán</Text>
        <Text style={tw`ml-auto text-sm text-[#00A982] font-semibold`}>
          ₫110.000
        </Text>
        <TouchableOpacity style={tw`ml-2`}>
